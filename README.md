...
|---|---|---|
| Server | `baseURL`, `model`, `displayName`, `apiKey` (`DSH_QWEN38_BASE_URL` / `_MODEL` / `_DISPLAY_NAME` / `_API_KEY`) | `http://localhost:8082/v1`, `qwen3.8-27b-nvfp4` (ninfer) / `qwen3.8-27b.gguf` (llama.cpp), same as `model`, — |
...
| 区域 | 字段（环境变量） | 默认 |
|---|---|---|
| 服务器 | `baseURL`、`model`、`displayName`、`apiKey`（`DSH_QWEN38_BASE_URL` / `_MODEL` / `_DISPLAY_NAME` / `_API_KEY`） | `http://localhost:8082/v1`、`qwen3.8-27b-nvfp4`（ninfer）/ `qwen3.8-27b.gguf`（llama.cpp）、同 `model`、— |
